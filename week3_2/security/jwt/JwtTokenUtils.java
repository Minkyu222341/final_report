...
public final class JwtTokenUtils {
    ...
    public static String generateJwtToken(UserDetailsImpl userDetails) {
        ...
        System.out.println("엑세스토큰 생성");
        return token;
    }

    public static String generateRefreshToken(UserDetailsImpl userDetails) {
        String refreshToken = null;
        long expireTime = System.currentTimeMillis() + REFRESH_TOKEN_EXPIRE_TIME;
        try {
            refreshToken = JWT.create()
                    .withClaim(CLAIM_USER_NAME, userDetails.getUsername())
                    .withClaim(CLAIM_EXPIRED_DATE, new Date(expireTime))
                    .sign(generateAlgorithm());
        } catch (Exception e) {
            System.out.println(e.getMessage());
        }
        System.out.println("리프레쉬토큰 생성");
        return refreshToken;
    }
